...
def start_all():
    ...
    dummy_check_result = firewall.tasks.kernel.modprobe.Check().delay('dummy')
    ...
    if not dummy_check_result.get():
        ...
        dummy_active_result = firewall.tasks.kernel.modprobe.Add().delay('dummy', '')
    ...
    logger.info(log_pref + 'Check si le module est chargé')
    bridge_check_result = firewall.tasks.kernel.modprobe.Check().delay('bridge')
    ...
    if not bridge_check_result.get():
        logger.info(log_pref + 'Activation du module')
        bridge_active_result = firewall.tasks.kernel.modprobe.Add().delay('bridge', '')
